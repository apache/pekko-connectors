...
public class MqttSourceTest {
  ...
  public void receiveUserProperties() throws Exception {
    ...
    MqttConnectionSettings connectionSettings =
        MqttConnectionSettings.create(
            "tcp://localhost:1883", "test-java-user-props", new MemoryPersistence());
    ...
    MqttMessage msg =
        MqttMessage.create(topic, ByteString.fromString("test"))
            .withUserProperties(userPropsToSend);
    Sink<MqttMessage, CompletionStage<Done>> mqttSink =
        MqttSink.create(
            connectionSettings.withClientId("source-test/user-props-sink"), MqttQoS.atLeastOnce());
