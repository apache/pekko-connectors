...
import org.apache.pekko.stream.connectors.google.{ GoogleSettings, PaginatedRequest, ResumableUpload }
...
private[connectors] trait Google {
   ...
   *
   * @param request the [[org.apache.pekko.http.scaladsl.model.HttpRequest]] to make
   ...
   *
   * @param request the initial [[org.apache.pekko.http.scaladsl.model.HttpRequest]] to make; must be a `GET` request
   ...
   *
   * @param request the [[org.apache.pekko.http.scaladsl.model.HttpRequest]] to initiate the upload; must be a `POST` request with query `uploadType=resumable`
   *                and optionally a [[org.apache.pekko.stream.connectors.google.scaladsl.`X-Upload-Content-Type`]] header
   * @tparam Out the data model for the resource
   * @return a [[org.apache.pekko.stream.scaladsl.Sink]] that materializes a [[scala.concurrent.Future]] containing the unmarshalled resource
   */
