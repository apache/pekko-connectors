...
import org.apache.pekko.stream.connectors.google.scaladsl.{ Google => ScalaGoogle }
...
private[connectors] trait Google {
   ...
   * Retries the request if the unmarshaller throws a [[org.apache.pekko.stream.connectors.google.util.Retry]].
   *
   * @param request the [[org.apache.pekko.http.javadsl.model.HttpRequest]] to make
   ...
   *
   * @param request the initial [[org.apache.pekko.http.javadsl.model.HttpRequest]] to make; must be a `GET` request
   * @tparam Out the data model for each page of the resource
   * @return a [[org.apache.pekko.stream.javadsl.Source]] that emits an `Out` for each page of the resource
   */
   ...
   * If the unmarshaller throws a [[org.apache.pekko.stream.connectors.google.util.Retry]] the upload will attempt to recover and continue.
   *
   * @param request the [[org.apache.pekko.http.javadsl.model.HttpRequest]] to initiate the upload; must be a `POST` request with query `uploadType=resumable`
   *                and optionally a [[org.apache.pekko.stream.connectors.google.javadsl.XUploadContentType]] header
   * @tparam Out the data model for the resource
   * @return a [[org.apache.pekko.stream.javadsl.Sink]] that materializes a [[java.util.concurrent.CompletionStage]] containing the unmarshalled resource
   */
