...
object Dependencies {
  ...
  val GoogleBigQueryStorage = Seq(
    ...
    libraryDependencies ++= Seq(
      ...
      "com.google.api.grpc" % "proto-google-cloud-bigquerystorage-v1" % "3.19.1" % "protobuf-src",
  ...
  val GooglePubSubGrpc = Seq(
    ...
    libraryDependencies ++= Seq(
      ...
      "com.google.cloud" % "google-cloud-pubsub" % "1.147.0" % "protobuf-src",
      "io.grpc" % "grpc-auth" % org.apache.pekko.grpc.gen.BuildInfo.grpcVersion,
  ...
  val Kinesis = Seq(
    libraryDependencies ++= Seq(
      ...
      "software.amazon.kinesis" % "amazon-kinesis-client" % "3.3.0").map(
      _.excludeAll(
        ExclusionRule("software.amazon.awssdk", "apache-client"),
