...
class JmsConnectorsSpec extends JmsSpec {
  ...
  "The JMS Connectors" should {
    "publish and consume strings through a queue" in withServer() { server =>
      val url = server.getBrokerUrl
      // #connection-factory
      // #text-sink
      // #text-source
      val connectionFactory: jakarta.jms.ConnectionFactory = new ActiveMQConnectionFactory(url)
      // #connection-factory
      // #text-sink
      // #text-source
    ...
    "disconnection should fail the stage after exhausting retries" in withServer() { server =>
      ...
      val result = JmsConsumer(
        JmsConsumerSettings(consumerConfig, connectionFactory)
          .withQueue("test")
          .withConnectionRetrySettings(ConnectionRetrySettings(system).withMaxRetries(3))).runWith(Sink.seq)
      Thread.sleep(500)
      server.stop()
    ...
    "only fail after maxBackoff retry" in withServer() { server =>
      ...
      val result = JmsConsumer(
        JmsConsumerSettings(consumerConfig, connectionFactory)
          ...
          .withQueue("test")).runWith(Sink.seq)
    ...
    "fail if message destination is not defined" in {
      val connectionFactory = new ActiveMQConnectionFactory("vm://localhost:1234")
  ...
  "support request/reply with temporary queues" in withConnectionFactory() { connectionFactory =>
    ...
    val tempQueueDest = connectors.jakartams.Destination(tempQueue)
