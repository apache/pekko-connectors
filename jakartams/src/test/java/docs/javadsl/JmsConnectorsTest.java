...
public class JmsConnectorsTest {
    ...
    private List<JmsTextMessage> createTestMessageList() {
        ...
        for (Integer n : intsIn) {
            ...
            JmsTextMessage message =
                    org.apache.pekko.stream.connectors.jakartams.JmsTextMessage.create(n.toString())
    ...
    public void publishAndConsumeJmsTextMessage() throws Exception {
        withServer(
                server -> {
                    ...
                    // #text-sink
                    // #text-source
                    ConnectionFactory connectionFactory = new ActiveMQConnectionFactory(server.getVmURL());
                    // #text-source
                    // #connection-factory
                    // #text-sink
    ...
    public void publishAndConsumeJmsObjectMessage() throws Exception {
        withServer(
                server -> {
                    ...
                    // #object-sink
                    // #object-source
                    ActiveMQConnectionFactory connectionFactory = new ActiveMQConnectionFactory(server.getVmURL());
    ...
    @Test
    public void useCustomDestination() throws Exception {
        withConnectionFactory(
                connectionFactory -> {
                    Sink<JmsTextMessage, CompletionStage<Done>> jmsSink =
                            JmsProducer.sink(
                ...
                connectionFactory -> {
                    ...
                    List<String> resultText =
                            result.toCompletableFuture().get().stream()
                                    .map(
                                            message -> ((ActiveMQTextMessage) message).getText())
                                    .collect(Collectors.toList());
    ...
    public void publishAndConsumeDurableTopic() throws Exception {
        withServer(
                server -> {
                    ConnectionFactory producerConnectionFactory = new ActiveMQConnectionFactory(server.getVmURL());
                    // #create-connection-factory-with-client-id
                    ConnectionFactory consumerConnectionFactory = new ActiveMQConnectionFactory(server.getVmURL());
        ...
        withConnectionFactory(
                connectionFactory -> {
                    ...
                    Destination tempQueueDest =
                            org.apache.pekko.stream.connectors.jakartams.Destination.createDestination(tempQueue);
                                    ...
                                    .map(
                                            textMessage -> {
                                                ...
                                                for (JmsHeader h : textMessage.getHeaders())
                                                    ...
                                                    else if (h.getClass().equals(JmsCorrelationId.class))
                                                        m = m.withHeader(h);
                                                return m;
                                            })
                                    .via(
