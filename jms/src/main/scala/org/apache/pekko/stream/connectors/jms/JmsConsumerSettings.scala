...
object JmsConsumerSettings {
  ...
  val configPath = "pekko.connectors.jms.consumer"
  ...
  /**
   * Reads from the default config provided by the actor system at `pekko.connectors.jms.consumer`.
   *
  ...
  /**
   * Java API: Reads from the default config provided by the actor system at `pekko.connectors.jms.consumer`.
   *
   * @param actorSystem The actor system
   * @param connectionFactory Factory to use for creating JMS connections.
   */
