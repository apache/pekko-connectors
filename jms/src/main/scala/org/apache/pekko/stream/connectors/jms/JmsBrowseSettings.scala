...
object JmsBrowseSettings {
  ...
  val configPath = "pekko.connectors.jms.browse"
  ...
  /**
   * Reads from the default config provided by the actor system at `pekko.connectors.jms.browse`.
   *
  ...
  /**
   * Java API: Reads from the default config provided by the actor system at `pekko.connectors.jms.browse`.
   *
   * @param actorSystem The actor system
   * @param connectionFactory Factory to use for creating JMS connections.
   */
