...
final case class UnsupportedMessagePropertyType(propertyName: String, propertyValue: Any, message: JmsEnvelope[_])
    extends Exception(
...
final case class NullMessageProperty(propertyName: String, message: JmsEnvelope[_])
    extends Exception(
      s"null value was given for Jms property '$propertyName'.")
    with NonRetriableJmsException

final case class UnsupportedMapMessageEntryType(entryName: String, entryValue: Any,
...
@deprecated("Not used anywhere", "Alpakka 3.0.4")
final case class NullMapMessageEntry(entryName: String, message: JmsMapMessagePassThrough[_])
    extends Exception(
      s"null value was given for Jms MapMessage entry '$entryName'.")
    with NonRetriableJmsException

final case class UnsupportedMessageType(message: jms.Message)
    extends Exception(
      s"Can't convert a ${message.getClass.getName} to a JmsMessage")
    with NonRetriableJmsException

final case class ConnectionRetryException(message: String, cause: Throwable) extends Exception(message, cause)
...
final case class JmsConnectTimedOut(message: String) extends TimeoutException(message)
