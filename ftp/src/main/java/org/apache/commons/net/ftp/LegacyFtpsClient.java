...
/**
 * This class is a workaround for code introduced in commons-net 3.9.0 that breaks FTPS support when
 * HTTP proxies are used. See https://issues.apache.org/jira/browse/NET-718
 *
 * <p>Derived from
 * https://github.com/apache/commons-net/blob/master/src/main/java/org/apache/commons/net/ftp/FTPSClient.java
 * and
...
public final class LegacyFtpsClient extends FTPSClient {
   ...
   *
   * <p>Sets protocol to {@link #DEFAULT_PROTOCOL} - i.e. TLS
   *
  ...
  private Socket _openFTPDataConnection_(final String command, final String arg)
      throws IOException {
    ...
    if (dataConnectionMode == ACTIVE_LOCAL_DATA_CONNECTION_MODE) {
      ...
      try (final ServerSocket server =
        ...
        } else if (!FTPReply.isPositiveCompletion(
