...
object HTableStage {
   ...
   */
  def sink[A](config: HTableSettings[A]): org.apache.pekko.stream.javadsl.Sink[A, CompletionStage[Done]] =
   ...
   * HBase mutations for every incoming element are derived from the converter functions defined in the config.
   */
  def flow[A](settings: HTableSettings[A]): org.apache.pekko.stream.javadsl.Flow[A, A, NotUsed] =
    Flow.fromGraph(new HBaseFlowStage[A](settings)).asJava

  /**
   * Reads an element from HBase.
   */
  def source[A](scan: Scan, settings: HTableSettings[A]): org.apache.pekko.stream.javadsl.Source[Result, NotUsed] =
